...
bool XMLUtil::ToBit8(const char* str, char* value)
{
    char n = 0;
    int count = 8;
    if (strnlen(str, 8) < 8)
...
bool XMLUtil::ToBit16(const char* str,  short* value)
{
    unsigned short n = 0;
    int count = 16;
    if (strnlen(str, 16) < 16)
    {
        count = strnlen(str, 16);
    }
    //这个做转换
    for (int i = 0; i < count; i++)
    {
...
bool XMLUtil::ToBit32(const char* str, int* value)
{
    unsigned int n = 0;
    int count = 32;
    if (strnlen(str, 32) < 32)
    {
        count = strnlen(str, 32);
    }
    //这个做转换
    for (int i = 0; i < count; i++)
    {
...
bool XMLUtil::ToBit64(const char* str, long long* value)
{
    unsigned long long n = 0;
    int count = 64;
    if (strnlen(str, 64) < 64)
    {
        count = strnlen(str, 64);
    }
    //这个做转换
...
XMLError XMLAttribute::QueryBit8Value(char* bvalue) const
{
    if (XMLUtil::ToBit8( Value(), bvalue)) {
        return XML_SUCCESS;
    }
    return XML_WRONG_ATTRIBUTE_TYPE;
}
XMLError XMLAttribute::QueryBit16Value(short* bvalue) const
{
    if (XMLUtil::ToBit16(Value(), bvalue)) {
        return XML_SUCCESS;
    }
    return XML_WRONG_ATTRIBUTE_TYPE;
}
XMLError XMLAttribute::QueryBit32Value(int* bvalue) const
{
    if (XMLUtil::ToBit32(Value(), bvalue)) {
        return XML_SUCCESS;
    }
    return XML_WRONG_ATTRIBUTE_TYPE;
}
XMLError XMLAttribute::QueryBit64Value(long long* bvalue) const
{
    if (XMLUtil::ToBit64(Value(), bvalue)) {
        return XML_SUCCESS;
    }
    return XML_WRONG_ATTRIBUTE_TYPE;
}
...
XMLError XMLElement::QueryBit8Text(char* bval) const
{
    if (FirstChild() && FirstChild()->ToText()) {
        const char* t = FirstChild()->Value();
        if (XMLUtil::ToBit8(t, bval)) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
XMLError XMLElement::QueryBit16Text(short* sval) const
{
    if (FirstChild() && FirstChild()->ToText()) {
        const char* t = FirstChild()->Value();
        if (XMLUtil::ToBit16(t, sval)) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
XMLError XMLElement::QueryBit32Text(int* ival) const
{
    if (FirstChild() && FirstChild()->ToText()) {
        const char* t = FirstChild()->Value();
        if (XMLUtil::ToBit32(t, ival)) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
XMLError XMLElement::QueryBit64Text(long long * lval) const
{
    if (FirstChild() && FirstChild()->ToText()) {
        const char* t = FirstChild()->Value();
        if (XMLUtil::ToBit64(t, lval)) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
...
char XMLElement::Bit8Text() const
{
    char b = 0;
    QueryBit8Text(&b);
    return (char)b;
}
short XMLElement::Bit16Text() const
{
    short b = 0;
    QueryBit16Text(&b);
    return (short)b;
}
int XMLElement::Bit32Text() const
{
    int b = 0;
    QueryBit32Text(&b);
    return (int)b;
...
long long XMLElement::Bit64Text() const
{
    long long b = 0;
    QueryBit64Text(&b);
    return (long long)b;
